...
function cargarTabla(permisoEditar, permisoExportar, permisoCambiar) {
	...
	if (permisoCambiar == "si") {
		btnAcciones +=
			'<button type="button" id="btnCambiarEstado" class="btn btn-info btn-sm"><i class="glyphicon glyphicon-refresh"></i></button>';
	}

	if (permisoEditar == "si") {
		btnAcciones +=
			'<button type="button" id="btnEditarUsuario" class="btn btn-info btn-sm" data-toggle="modal" data-target="#modaleditarUsuario"><i class="glyphicon glyphicon-pencil"></i></button>';
	}
	...
	if (permisoExportar == "si") {
		$(".dataTables-usuarios").DataTable({
			...
			ajax: {
				url: base_url + "getListadoUsuarios",
				type: "GET",
			},
			columnDefs: [
				{
...
function getDetalleUsuario(idUsuario) {
	$.ajax({
		...
		$.each(msg, function (i, o) {
			fila +=
				'<div class="col-md-12"><label for="nombre">NOMBRE DE USUARIO</label><input type="text" class="form-control custom-input-sm" id="name" value="' +
				o.atr_nombre +
				'"></div>';
			fila +=
				'<div class="col-md-12"><br><label for="nombre">CORREO ELECTRÓNICO</label><input type="text" class="form-control custom-input-sm" id="email" value="' +
				o.atr_correo +
				'"></div>';
			fila +=
				'<div class="col-md-6 col-sm-12"><br><label for="nombre">CONTRASEÑA</label><input type="password" class="form-control custom-input-sm" id="pass1"></div>';
			fila +=
				'<div class="col-md-6 col-sm-12"><br><label for="nombre">CONFIRMAR CONTRASEÑA</label><input type="password" class="form-control custom-input-sm" id="pass2"></div>';
			fila +=
				'<div class="col-md-12"><br><label for="getSelectPerfilesEditar">PERFIL</label><select class="custom-select" id="getSelectPerfilesEditar">';
			...
			$.getJSON(url, function (result) {
				...
				fila += "</select></div>";
				fila +=
					'<label id="idUser" style="display:none">' + idUsuario + "</label>";

...
function cargarTablaModelos(permisoEditar, permisoExportar) {
	...
	if (permisoEditar == "si") {
		btnAcciones =
			'<button type="button" id="getDetalleModelo" class="btn btn-info btn-sm" data-toggle="modal" data-target="#modalEditarModelo"><i class="glyphicon glyphicon-pencil"></i></button>';
	}
	...
	if (permisoExportar == "si") {
		$(".dataTables-modelos").DataTable({
			...
			ajax: {
				url: "https://www.imlchile.cl/dev_test/grupofirma/index.php/getListadoModelos",
				type: "GET",
			},
			columnDefs: [
				{
...
function getDetalleModelo(idModelo) {
	$.ajax({
		...
		$.each(msg.msg, function (i, o) {
			fila +=
				'<h5 class="modal-title mx-auto">MODELO</h5><button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>';
			...
			$.getJSON(url, function (result) {
				...
				fila +=
					...
					'"></div>';
				fila +=
					'<label style="display:none" id="labelMarca">' +
					o.cf_marca +
					'</label><label style="display:none" id="labelModelo2">' +
					idModelo +
					"</label>";

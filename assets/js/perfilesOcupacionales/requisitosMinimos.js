...
function cargarTabla(cargo, permisoEliminar) {
	...
	if (permisoEliminar == "si") {
		btnAcciones =
			'<button type="button" id="btnEliminarRequisitoMínimo" class="btn btn-danger btn-sm" data-toggle="modal" data-target="#modalEliminarRequisitoMinimo"><i class="glyphicon glyphicon-trash"></i></button>';
	}
	...
	$(".dataTables-requerimientosMinimos").DataTable({
		...
		ajax: {
			url:
				"http://www.imlchile.cl/dev_test/grupofirmaindex.php/getListadoRequisitosMinimosDataTable?id=" +
				cargo,
			type: "GET",
		},
		columnDefs: [
...
function eliminarRequisitoMinimo(idRequisitoMinimo) {
	...
	$.ajax({
		...
		type: "POST",
		dataType: "json",
		data: { idRequisitoMinimo: idRequisitoMinimo, cargo: cargo },
	}).then(function (msg) {
...
function agregarListaDeRequisitosMinimos() {
	...
	for (var i = 0; i < constante; i++) {
		...
		if (requisito != "") {
			$.ajax({
				...
				type: "POST",
				dataType: "json",
				data: { requisitoMinimo: requisito, cargo: cargo },
			}).then(function (msg) {
				if (msg.msg == "ok") {
	...
	document.getElementById("btnAgregarListaDeRequisitosMinimos").style.display =
		"none";
...
function bloquearBoton() {
	...
	document
		.getElementById("btnAgregarListaDeRequisitosMinimos")

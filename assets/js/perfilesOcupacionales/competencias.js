...
function cargarTabla(cargo, permisoEliminar) {
	...
	if (permisoEliminar == "si") {
		btnAcciones =
			'<button type="button" id="btnEliminarCompetencias" class="btn btn-danger btn-sm" data-toggle="modal" data-target="#modalEliminarRequisitoMinimo"><i class="glyphicon glyphicon-trash"></i></button>';
	}
	...
	$(".dataTables-competencias").DataTable({
		...
		ajax: {
			url:
				"https://www.imlchile.cl/dev_test/grupofirma/index.php/getListadoCompetenciasDataTable?id=" +
				cargo,
			type: "GET",
		},
		columnDefs: [
...
function agregarListaDeCompetencias() {
	...
	for (var i = 0; i < constante; i++) {
		...
		if (competencia != "") {
			$.ajax({
				...
				type: "POST",
				dataType: "json",
				data: { competencia: competencia, cargo: cargo },
			}).then(function (msg) {
				if (msg.msg == "ok") {
	...
	//Se inicializa en 0 para que al cambiar de cargo los inputs nuevamente comiencen desde 0
	constante = 0;
	document.getElementById("btnAgregarListaDeCompetencias").style.display =
		"none";
}
...
function bloquearBoton() {
	...
	document
		.getElementById("btnAgregarListaDeCompetencias")

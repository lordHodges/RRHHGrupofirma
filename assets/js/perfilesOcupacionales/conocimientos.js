...
function cargarTabla(cargo, permisoEliminar) {
	...
	if (permisoEliminar == "si") {
		btnAcciones =
			'<button type="button" id="btnEliminarConocimiento" class="btn btn-danger btn-sm"><i class="glyphicon glyphicon-trash"></i></button>';
	}
	...
	$(".dataTables-conocimientos").DataTable({
		...
		ajax: {
			url:
				"https://www.imlchile.cl/dev_test/grupofirma/index.php/getListadoConocimientosDataTable?id=" +
				cargo,
			type: "GET",
		},
		columnDefs: [
...
function agregarListaDeConocimientos() {
	...
	for (var i = 0; i < constante; i++) {
		...
		if (conocimiento != "") {
			$.ajax({
				...
				type: "POST",
				dataType: "json",
				data: { conocimiento: conocimiento, cargo: cargo },
			}).then(function (msg) {
				if (msg.msg == "ok") {
	...
	constante = 0;
	$("#conocimientos").empty();
	document.getElementById("btnAgregarListaDeConocimientos").style.display =
		"none";
...
function bloquearBoton() {
	...
	document
		.getElementById("btnAgregarListaDeConocimientos")

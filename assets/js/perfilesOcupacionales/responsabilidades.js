	...
	if (nombre == "" || jefeDirecto == "") {
		...
		$.ajax({
			...
			if (msg.msg == "ok") {
				...
				document
					.getElementById("btnAgregarInputResponsabilidad")
...
function getDetalleCargo(id) {
	...
	$.ajax({
		...
		$.each(response.msg.array_cargo, function (i, o) {
			fila +=
				'<div class="col-md-12"><br>  <label for="nombreNuevo">NOMBRE:  &nbsp;</label> <label id="idCargo" style="color:#2a3f54">' +
				o.cp_cargo +
				'</label> <input type="text" style="color:#848484" class="form-control  custom-input-sm" id="nombreNuevo"></div>';

			var url = base_url + "getSucursales";
			fila +=
				'<div class="col-md-12"><br><label for="sucursal">SUCURSAL:&nbsp;</label><label id="sucursalActual">' +
				o.sucursal +
				'</label><select class="custom-select" id="getSelectSucursal2"> ';
			...
			$.getJSON(url, function (result) {
				...
				fila +=
					'<div class="col-md-10"><br><label >RESPONSABILIDADES PRINCIPALES &nbsp</label><button type="button" class="btn btn-success btn-sm center"  id="btnAgregarInputResponsabilidadEditar" ><i class="glyphicon glyphicon-plus"></i></button></div>';
				...
				$.each(response.msg.array_responsabilidades, function (i, r) {
					fila +=
						...
						r.atr_descripcion +
						'" onclick="deleteResponsabilidad(this)"><i class="glyphicon glyphicon-minus"></i></button>&nbsp;<label id="responsabilidadActual_' +
						contadorResponsabilidades +
						'">' +
						r.atr_descripcion +
						'</label><input type="text" id="responsabilidadNuevo_' +
						contadorResponsabilidades +
						'"  class="form-control custom-input-sm" id="responsabilidadNuevo_' +
						r.cp_responsabilidad +
						'"></div>';
				...
				fila +=
					'<div class="col-md-12"><br><label for="jefeDirectoNuevo">JEFE DIRECTO:  &nbsp;</label>  <input type="text" style="color:#848484" class="form-control custom-input-sm" id="jefeDirectoNuevo"></div>';
				fila +=
					'<div class="col-md-12"><br><label for="lugarTrabajoNuevo">LUGAR DE PRESTACIÓN DE SERVICIOS:  &nbsp;</label>  <textarea type="text"  style="color:#848484" class="form-control" rows="5" id="lugarTrabajoNuevo"></textarea></div>';
				fila +=
					'<div class="col-md-12"><br><label for="jornadaTrabajoNuevo">JORNADA DE TRABAJO:  &nbsp;</label>  <textarea type="text" style="color:#848484" class="form-control" rows="5" id="jornadaTrabajoNuevo"></textarea></div>';
				fila +=
					'<div class="col-md-12"><br><label for="diasTrabajoNuevo">DÍAS DE TRABAJO:  &nbsp;</label>  <input type="text" style="color:#848484" class="form-control custom-input-sm" id="diasTrabajoNuevo"></div>';
				...
				document.getElementById("jornadaTrabajoNuevo").value =
					o.atr_jornadaTrabajo;
	...
	for (var i = 0; i < constante; i++) {
		...
		if (responsabilidad != "") {
			$.ajax({
				...
				type: "POST",
				dataType: "json",
				data: { responsabilidad: responsabilidad, cargo: id },
			}).then(function (msg) {
...
function bloquearBoton() {
	...
	document.getElementById("btnAgregarInputResponsabilidad").style.display =
		"none";
...
function bloquearBotonEditar() {
	...
	document.getElementById(
		"btnAgregarInputResponsabilidadEditar"
	).style.display = "none";
}
...
function agregaInputResponsabilidad() {
	...
	var count = contar();
	fila.innerHTML +=
		'<div class="col-md-12" style="margin-top:10px; color:#848484;"><input type="text" class="form-control custom-input-sm " style="color:#848484" nkeypress="bloquearBoton()" id="input_tarea' +
		count +
		'" required></div>';
}
...
function agregaInputResponsabilidadEditar() {
	...
	var count = contar();
	fila.innerHTML +=
		'<div class="col-md-12" style="margin-top:10px; color:#848484"><input type="text" class="form-control custom-input-sm " onkeypress="bloquearBotonEditar()"  style="color:#848484" id="input_tarea' +
		count +
		'" required></div>';
}

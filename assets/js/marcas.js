...
function cargarTablaMarcas(permisoEditar, permisoExportar) {
	...
	if (permisoEditar == "si") {
		btnAcciones =
			'<button type="button" id="getDetalleMarca" class="btn btn-info btn-sm" data-toggle="modal" data-target="#modalEditarMarca"><i class="glyphicon glyphicon-pencil"></i></button>';
	}
	...
	if (permisoExportar == "si") {
		$(".dataTables-marcas").DataTable({
			...
			ajax: {
				url: "https://www.imlchile.cl/dev_test/grupofirmaindex.php/getListadoMarcas",
				type: "GET",
			},
			columnDefs: [
				{
...
function getDetalleMarca(idMarca) {
	$.ajax({
		...
		$.each(msg.msg, function (i, o) {
			fila +=
				'<h5 class="modal-title mx-auto">MARCA</h5><button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>';
			fila +=
				'<div class="col-md-12"><br><label for="nombre">NOMBRE&nbsp; </label><label id="idMarca" style="color:#2A3F54;">' +
				o.cp_marca +
				'</label><input type="text" style="color:#848484" class="form-control custom-input-sm" oninput="mayus(this)" id="nombreEditar" value="' +
				o.atr_descripcion +
				'"></div>';
			fila +=
				'<label id="labelMarca" style="display:none">' + idMarca + "</label>";
...
function getSelectMarcas() {
	...
	$.getJSON(url, function (result) {
		$.each(result, function (i, o) {
			fila +=
				"<option value='" + o.cp_marca + "'>" + o.atr_descripcion + "</option>";
		});

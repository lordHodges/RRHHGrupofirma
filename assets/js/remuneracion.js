...
function getDetalleRemuneracion(id){
  ...
  $.ajax({
      ...
      $.each(response.msg.array_remuneracion, function (i, o) {
        fila +='<h5 class="modal-title mx-auto">REMUNERACIÓN</h5><button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>';

        fila +='<div class="col-md-12"><h5 id="cargoActual" class="text-center" style="color:#fafafa;">'+o.atr_nombre+'</h5><label id="idCargo" style="color:#2A3F54; position:absolute; margin-top:0px;">'+o.cp_cargo+'</label></div>';

        fila +='<div class="col-md-12"><br><label for="nombre">Sueldo mensual:&nbsp;$ </label><label id="sueldoMensualActual">'+o.atr_sueldoMensual+'</label><input type="text" onkeyup="this.value=soloNumeros(this.value); formatoMiles(this);" class="form-control custom-input-sm" id="sueldoMensualNuevo"></div>';

        fila +='<div class="col-md-12"><br><label for="imposiciones">Imposiciones</label><select class="custom-select" id="getSelectImposiciones"><option disabled selected >Seleccione una opción</option><option value="1">Si</option><option value="0">No</option></select></div>';

        fila +='<div class="col-md-12"><br><label for="nombre">Colación:&nbsp;$ </label><label id="colacionActual">'+o.atr_colacion+'</label><input type="text" class="form-control custom-input-sm" onkeyup="this.value=soloNumeros(this.value); formatoMiles(this);" id="colacionNuevo"></div>';
...
function agregaInputRemuneracionesExtra() {
   ...
   var count = contar();
   fila.innerHTML += '<div class="col-md-12" style="margin-top:10px; color:#848484"><input type="text" class="form-control custom-input-sm " onkeypress="btnAgregarInputRemuneracionesExtra()" id="input_tarea'+count+'" required></div>';
}
...
function btnAgregarInputRemuneracionesExtra(){
    ...
    document.getElementById('btnAgregarInputRemuneracionesExtra').style.display = 'none';
}
